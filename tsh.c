...
void eval(char *cmdline) 
{
  ...
  pid_t pid = -1;       /* process pid */
  sigset_t mask;        /* mask to avoid race condition */
  ...
  if (!builtin_cmd(argv1)) {
    ...
    pid = fork();
    if (pid == 0) {
      // Put the child in a new process group whose group ID is the child's PID
      setpgid(0, 0);
  ...
  // Wait for job to terminate if it's a foreground process, otherwise just add it to the job list
  if (!bg) {
    if (addjob(jobs, pid, FG, cmdline)) {
      sigprocmask(SIG_UNBLOCK,&mask,NULL);
      waitfg(pid);
    } else {
      // Kill the child if addjob was unsuccessful
      kill(-pid, SIGINT);
    }
  } else {
